...
def ingest_stations_data(**context):
    ...
    month = ctx["month"]
    logger.info(
        "Execution context retrieved - Date: %s, Year: %s, Month: %s",
        execution_date,
        year,
        month,
    )
    ...
    logger.info("Azure credentials loaded - Storage Account: %s", storage_account_name)
    ...
    # Use temp folder for initial upload
    ...
    logger.info("Container client created for temp container: %s", temp_container_name)
    ...
    try:
        ...
        logger.error(
            "Failed to fetch data from URL: %s - Error: %s - Status Code: %s",
            url,
            str(e),
    ...
    logger.info(
        "XCom values pushed - year: %s, month: %s, blob_name: %s, file_size_mb: %.2f",
        year,
        month,
        blob_name,
        file_size_mb,
