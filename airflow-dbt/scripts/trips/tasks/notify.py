...
def send_success_notification(**context):
    ...
    year = context['task_instance'].xcom_pull(task_ids='ingest_trips_data', key='year')
    month = context['task_instance'].xcom_pull(task_ids='ingest_trips_data', key='month')
    row_count = context['task_instance'].xcom_pull(task_ids='validate_data', key='row_count')
    
    message = f"""
    ✅ Citibike Monthly Ingestion Successful
...
def send_failure_notification(**context):
    ...
    message = f"""
    ❌ Citibike Monthly Ingestion Failed
